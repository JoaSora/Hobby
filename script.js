...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function mostrarItems(tipo) {
    ...
    visibles.forEach((item, realIndex) => {
      ...
      // Agregar clase de fondo según tipo o platino
      let claseFondo = '';
      if (tipo === 'lectura') {
      claseFondo = `fondo-${item.tipo}`;  // libro, manga, manhwa, novela
      } else if (tipo === 'visualizacion') {
      claseFondo = `fondo-${item.tipo}`;  // pelicula, serie, anime
      } else if (tipo === 'juegos') {
      claseFondo = `fondo-platino-${item.platino}`;  // Si, No, Proceso
      }
      card.classList.add(claseFondo);
      card.draggable = true;
